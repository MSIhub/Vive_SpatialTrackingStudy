...
struct _InitFlags
{
		...
		bool printTransformationMatrix = true;
};
...
public:
	...
	*/
	bool RunProcedure(SOCKET ClientSocket, std::ofstream& file_hmd, std::ofstream& file_controller, std::ofstream& file_tracker);
	...
	*/
	void ParseTrackingFrame(SOCKET ClientSocket, std::ofstream& file_hmd, std::ofstream& file_controller, std::ofstream& file_tracker);
	...
	*/
	void GetPoseAllDevices(SOCKET ClientSocket);

	void HMDCoords(SOCKET ClientSocket, std::ofstream& file_hmd);
	...
	*/
	void ControllerCoords(SOCKET ClientSocket, std::ofstream& file_controller);
	...
	*/
	...
	void TrackerCoords(SOCKET ClientSocket, std::ofstream& file_tracker);
	...
	struct _ControllerData
	{
		//Fields to be initialzed by iterateAssignIds() and setHands()
   	  signed int deviceId = -1;   // Device ID according to the SteamVR system
