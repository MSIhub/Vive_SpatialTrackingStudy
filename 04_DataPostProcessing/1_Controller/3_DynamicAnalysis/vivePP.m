function [Vive] = vivePP(filenameHmd,filenameController3, filenameController4,filenameTracker, delimiter)
...
flnh = filenameHmd;
flnc3 = filenameController3;
flnc4 = filenameController4;
...
if isfile(flnc3)
    vr_controllers_raw = importdata(flnc3,delimiter);
...
if isfile(flnc4)
    vr_controllers_raw = importdata(flnc4,delimiter);
    if (isempty(vr_controllers_raw))
        disp("VR controllers file is empty, check whether the devices were connected")
    else
        controllerIDs = unique(vr_controllers_raw(:,1));
