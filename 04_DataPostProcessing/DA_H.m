...
for itr0 =1:1:length(velName)
    ...
    hep.(fieldName) = H_ep;
    heo.(fieldName) = H_eo;
    hvel.(fieldName) = H_vel;
    % Ztest and %% Maximum permissible error test
    TestDataset = {H_ep, H_eo};
    zComp = zeros(2,1);
    s_square = zeros(2,1);
    e_bar = zeros(2,1);
    e_L = zeros(2,1);
...
fields = fieldnames(hep);
legendName = ["$V_{1}$", "$V_{0.5}$", "$V_{0.4}$", "$V_{0.3}$", "$V_{0.250}$", "$V_{0.2}$", "$V_{0.185}$", "$V_{0.175}$", "$V_{0.150}$", "$V_{0.135}$", "$V_{0.125}$", "$V_{0.1}$", "$V_{0.05}$", "$V_{0.010}$"];
for itr2 =1:1:length(fields)
    % --- Plot data originally in dataset "Tracker"
    [CdfY,CdfX] = ecdf(hep.(fields{itr2}),'Function','cdf');  % compute empirical function
    hLine = stairs(CdfX,CdfY,'LineWidth',LineWidth);
    xlabel('Position error [mm]');
    ylabel('Cumulative probability')
    LegHandles(end+1) = hLine;
    LegText{end+1} = legendName{itr2};
end
...
set(gcf, 'Units', 'centimeters','Position',  [24.60625,19.596805555555555,9.68375,7.196666666666665], 'InnerPosition', [24.60625,19.596805555555555,9.68375,7.196666666666665], 'OuterPosition', [24.412222222222223,19.402777777777775,10.071805555555553,9.507361111111113]);
set(gcf,'Renderer', 'painters', 'RendererMode', 'manual');
fields = fieldnames(heo);
legendName = ["$V_{1}$", "$V_{0.5}$", "$V_{0.4}$", "$V_{0.3}$", "$V_{0.250}$", "$V_{0.2}$", "$V_{0.185}$", "$V_{0.175}$", "$V_{0.150}$", "$V_{0.135}$", "$V_{0.125}$", "$V_{0.1}$", "$V_{0.05}$", "$V_{0.010}$"];
for itr2 =1:1:length(fields)
    % --- Plot data originally in dataset "Tracker"
    [CdfY,CdfX] = ecdf(heo.(fields{itr2}),'Function','cdf');  % compute empirical function
    hLine = stairs(CdfX,CdfY,'LineWidth',LineWidth);
...
clearvars -except hvel hep heo delta_max
V = [];
E = [];
hfields = fieldnames(hvel);
for itr1 = 1:1:length(hfields)
    V = [V; hvel.(hfields{itr1})] ;
    E = [E; hep.(hfields{itr1})];
end
% V2 = V((V < 250)== 1);
% E2 = E((V < 250)== 1);
% scatter(V2,E2,'.')
