function [Vive] = vivePP(filenameHmd,filenameController,filenameTracker5,filenameTracker6,delimiter)
...
flnt5 = filenameTracker5;
flnt6 = filenameTracker6;
%%-------------------------------------------------------------------------%%
...
if isfile(flnt5)
    vr_trackers_raw5 = importdata(flnt5,delimiter);
...
if isfile(flnt6)
    vr_trackers_raw6 = importdata(flnt6,delimiter);
    % Extract and organise data from the file
    if (isempty(vr_trackers_raw6))
        disp("VR trackers file is empty, check whether the devices were connected. If no controllers are connected, the data will be in controller file")
    else
        trackerIDs = unique(vr_trackers_raw6(:,1));
        for itr3 = 1:1:length(trackerIDs)
            fld = strcat("tracker",string(trackerIDs(itr3)));
            rawVive.(fld) = vr_trackers_raw6(vr_trackers_raw6(:,1) ==  (trackerIDs(itr3)),:);
        end
    end
else
    disp("Tracker file is not available, check directory for file.")
